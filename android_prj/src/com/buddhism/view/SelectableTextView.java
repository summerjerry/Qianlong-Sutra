...
/**
 * User: ray Date: 2013-02-01 <p/> WARNING: SelectableTextView is designed to be used
 * inside a ScrollView. The selection functionality have only been tested under the
 * assumption that the parent of the view is a ScrollView. <p/> The functionality WILL
 * PROBABLY breaks if the TextView is not inside a ScrollView.
 */
public class SelectableTextView extends TextView {
  ...
  /**
   * set the selection beginning at {@code start} with the specified {@code length} for
  ...
  public void setSelection(int color, int start, int length, int duration) {
    ...
    this.mSelection = new SelectionInfo(this.getText(), new BackgroundColorSpan(color), start, end);
  ...
  /**
   * set the selection beginning at {@code start} with the specified {@code length} for
   * the given {@code duration} with the default color
   * @param start
   *          the start offset
   * @param length
   *          the length of the selection
  ...
  /**
   * Gets the character offset of (x, y). If (x, y) lies on the right half of the
   * character, it returns the offset of the next character. If (x, y) lies on the left
   * half of the character, it returns the offset of this character.
   * @param x
   ...
   * @return the character offset for the character whose position is closest to the
  ...
  /**
   * Get the offset character closest to the specified absolute position. If the resulting
   * offset is too close to the previous offset, return the previous offset instead.
   * @param x
   *          raw x
   * @param y
   *          raw y
  ...
  /**
   * Checks whether the specified offset is at the end of a line. <p/> PRECONDITION:
   * assumes layout exists and is valid
   * @param offset
   *          the offset to check
  ...
  /**
   * Get the (x,y) screen coordinate from the specified offset. If the specified offset is
   * beyond the end of the line, move the offset to the beginning of the next line.
   * @param offset
   *          the offset
   * @param scroll_y
  ...
  /**
   * Get the (x,y) screen coordinate from the specified offset. If the offset is the at
   * the end of a wrapped line, return the (x,y) at the end of that line instead of the
   * (x, y) at the beginning of the next line (which is the default behaviour for Android)
   * @param offset
   *          the offset
   * @param scroll_y
   *          the scroll distance to take away
  ...
  /**
   * Manages the two cursors that control the selection. Internal used only. For outsider,
   * please use SelectionModifier
   */
  private class SelectionCursorController implements ViewTreeObserver.OnTouchModeChangeListener {

    /**
     * The two cursor handle that controls the selection. Note that the two cursors are
     * allowed to swap positions and thus the name of the handle has no bearing on the
     * relative position of the handle to each other. (e.g. mStartHandle can be positioned
     * leagally at an offset greater than the offset mEndHandle is resting on)
     */
    ...
    /**
     * Select the textview between start and end. <p/> Precondition: start, end must be
     * valid
     * @param start
     *          the starting offset
     * @param end
  ...
  private class CursorHandle extends View {
    ...
    /**
     * the y coordinate of the "pointer" of the cursor which is usually the top, so it's
     * zero.
     */
    private int mHotspotY;

    /**
     * Adjustment to add to the Raw x, y coordinate of the touch position to get the
     * location of where the cursor is pointing to
     */
